...
function fmt(doc) {
  ...
  return {
    id:                doc._id.toString(),
    equipment_type_id: et ? et._id.toString() : doc.equipment_type_id?.toString(),
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!equipment_type_id || !client_name || !year_month)
      return res.status(400).json({ error: '缺少必填欄位：設備種類、客戶名稱、年月' })

    const doc = await RentalInvoice.create({
      equipment_type_id, client_name, vendor, site_name, year_month,
